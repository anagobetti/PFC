...
A primeira CNH só pode ser retirada nas categorias A ou B por meio de cursos teóricos preparatórios, médico e psicotécnico. Após a primeira habilitação existem algumas regras para mudança de categoria (\citet{DetranMG}):
...
Apesar de não ser o escopo do trabalho a identificação de falsificações no documento de CNH, é importante observar as diferenças entre os dois documentos, uma vez que dessa forma é possível identificar um algoritmo que atenda aos dois modelos. Pela análise da imagem \cite{fig:NOVACNH}, percebe-se que há uma grande alteração de tonalidades no novo modelo, mas os campos continuam seguindo a mesma ordem, desta forma as alterações realizadas não tem muito impacto no desenvolvimento deste trabalho. Além da clara mudança de cor, haverá um mapa do estado, inclusão de dois números de identificação nacional (Registro Nacional e número de espelho da CNH), entre outros dispositivos de segurança. A mudança para o novo modelo é gradual, portanto os motoristas não precisarão trocar o documento atual caso ele ainda seja válido, dessa forma os dois modelos irão coexistir por 5 anos. 
...
Na imagem \cite{fig:TiposImagem} pode-se observar um exemplo de cada categoria de imagem, sendo a primeira imagem à esquerda a imagem em escala RGB, que é a imagem colorida, a segunda imagem, ao centro, é a imagem em escala de cinza. Percebe-se que na imagem em escala de cinza a uma variação grande nas intensidade da cor de cinza, o que ainda permite a identificação de detalhes. Já a terceira imagem é a imagem binária, nela só a dois valores possíveis, 0 e 255, percebe-se que nesta imagem não é possível identificar os detalhes da imagem, como divisão entre grama e cerca facilmente. 
